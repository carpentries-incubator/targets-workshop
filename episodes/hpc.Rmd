...
```
To configure this for Slurm, we swap out the controller with a new one from the [`crew.cluster`](https://wlandau.github.io/crew.cluster/index.html) package:
...
`crew` uses a persistent worker strategy. This means that `crew` does not submit one Slurm job for each target. 
