...
#| label = "slurm-crew",
#| eval = FALSE,
#| code = readLines("files/plans/plan_slurm.R")[5:14]
```
...
There are a number of options you can pass to `crew_controller_slurm()` and `crew_options_slurm()` to fine-tune the Slurm execution, [which you can find here](https://wlandau.github.io/crew.cluster/reference/crew_controller_slurm.html) and [here](https://wlandau.github.io/crew.cluster/reference/crew_options_slurm.html) respectively.
Here we are only using three:
  ...
  * `memory_gigabytes_required` specifies the amount of memory we need.
...
::: hint
Check the arguments for [`crew_options_slurm`](https://wlandau.github.io/crew.cluster/reference/crew_options_slurm.html).
:::
::: solution
```R
tar_option_set(
...
::: callout
In general, it's better to use a dedicated `crew_options_slurm` argument than to use `script_lines`, if one exists.
...
```{R}
#| eval = FALSE,
#| code = readLines("files/plans/plan_slurm_memory.R")[9:22]
```
...
```{R}
#| eval = FALSE,
#| code = readLines("files/plans/plan_slurm_memory.R")[24:26]
tar_option_set(
...
```{R}
#| eval = FALSE,
#| code = readLines("files/plans/plan_slurm_memory.R")[28:43]
```
...
#| eval = FALSE,
#| code = readLines("files/plans/plan_slurm_gpu.R")[13:39]
```
```{R}
