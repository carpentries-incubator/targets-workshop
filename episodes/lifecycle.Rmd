...
library(targets)
library(visNetwork)
plan_01 <- here::here("episodes/files/_targets-01.R")
plan_02 <- here::here("episodes/files/_targets-02.R")
plan_03 <- here::here("episodes/files/_targets-03.R")
```
...
#| echo: FALSE
tar_dir({
  fs::file_copy(plan_02, "_targets.R")
...
tar_dir({
  ...
  tar_make(reporter = "silent")
  # New workflow
  fs::file_copy(plan_03, "_targets.R", overwrite = TRUE)
  # Run it again
  tar_make()
})
```
...
tar_dir({
  # New workflow
  fs::file_copy(plan_03, "_targets.R")
  # Run it
  tar_make(reporter = "silent")
  # Run it again
  tar_make(reporter = "silent")
...
```{r}
#| label: targets-run-show-4
#| eval: FALSE
tar_visnetwork()
```

```{r}
#| label: targets-run-hide-4
#| echo: FALSE
tar_file <- here::here("episodes/example_targets.R")
tar_dir({
  fs::file_copy(tar_file, "_targets.R")
  # Run it again
  tar_make()
  tar_visnetwork()
})
